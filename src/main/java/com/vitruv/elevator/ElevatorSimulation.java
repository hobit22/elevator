...
public class ElevatorSimulation {
	...
	public ElevatorSimulation(){
		...
		simTime = 0;
		simRate = 0;
		passengerBehavior =  new ArrayList<>(5);
	}
			...
			for(int j = 0; j < passengerBehavior.get(i).size();j++){
				...
				if (pa.getExpectedTimeOfArrival() == SimClock.getTime()){
					System.out.println("Time:  "+SimClock.getTime()+"  "+ pa.getNumPassengers() + 
							" 명의 승객 " + i + " 층에서 " + pa.getDestinationFloor() + " 층으로 가고싶음");
	...
	public void readFile() throws FileNotFoundException{
		...
		int floor = 0;
		while (s.hasNextLine()){
			passengerBehavior.add(new ArrayList<>());
			...
			for (int i = 0; i < parts.length;i++){
				...
				PassengerArrival PA = new PassengerArrival(passengers,destination,0,time);
